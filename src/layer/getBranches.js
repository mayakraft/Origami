 ...
 * @returns {string[][]}
 */
export const getBranches = (remainingKeys, constraints, lookup) => {
		...
		do {
			...
			tacoTypeNames.forEach((type) => {
				...
				indices
					...
					.map((faces) =>
						constraintToFacePairsStrings[type](faces)
							// add each facePair to the neighborsHash.
							.forEach((facePair) => {
								neighborsHash[facePair] = true;
							}),
					);
			});
			...
			neighbors.forEach((facePair) => {
				stackHash[facePair] = true;
			});
		} while (stack.length);
		i += 1;
		groups.push(group);

...
export const makeConstraintsLookup = (constraints) => {
	...
	tacoTypeNames.forEach((key) => {
		lookup[key] = {};
	});

	// fill every entry with an empty array
	tacoTypeNames.forEach((type) =>
		constraints[type].forEach((constraint) =>
			constraintToFacePairsStrings[type](constraint).forEach((key) => {
				lookup[type][key] = [];
			}),
		),
	);
	...
	// populate arrays
	tacoTypeNames.forEach((type) =>
		constraints[type].forEach((constraint, i) =>
			constraintToFacePairsStrings[type](constraint).forEach((key) =>
...
export const makeSolverConstraintsFlat = (
	...
	const { taco_taco, taco_tortilla, tortilla_tortilla } = makeTacosAndTortillas(
		{
			vertices_coords,
			edges_vertices,
			edges_faces,
	...
	const facePairs = connectedComponentsPairs(faces_facesOverlap).map((pair) =>
		pair.join(" "),
	);
